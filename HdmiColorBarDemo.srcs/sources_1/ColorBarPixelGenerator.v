...
reg [11:0] x_offset;
wire [11:0] cx = H_ACTIVE[11:0] >> 1;
wire [11:0] cy = V_ACTIVE[11:0] >> 1;
wire signed [12:0] dx = $signed({1'b0, active_x}) - $signed({1'b0, cx});
wire signed [12:0] dy = $signed({1'b0, active_y}) - $signed({1'b0, cy});
wire [12:0] abs_dx = dx[12] ? -dx : dx;
wire [12:0] abs_dy = dy[12] ? -dy : dy;
reg [2:0] sector;
reg [2:0] phase;
reg [4:0] rot_cnt;
reg [4:0] rot_cnt_next;
reg [2:0] phase_next;
reg [2:0] phase_reg;
wire [2:0] sector_rot = sector + phase;
assign hs = hs_reg_d0;
assign vs = vs_reg_d0;
...
always @(*) begin
    if (dx == 0 && dy == 0) begin
        sector = 3'd0; // 中心点归到 sector0
    end else if (dx >= 0 && dy >= 0) begin // Q1
        if (abs_dx >= abs_dy) sector = 3'd0; else sector = 3'd1;
    end else if (dx < 0 && dy >= 0) begin // Q2
        if (abs_dx <  abs_dy) sector = 3'd2; else sector = 3'd3;
    end else if (dx < 0 && dy < 0) begin  // Q3
        if (abs_dx >= abs_dy) sector = 3'd4; else sector = 3'd5;
    end else begin // dx >=0 && dy < 0  Q4
        if (abs_dx <  abs_dy) sector = 3'd6; else sector = 3'd7;
    end
end
...
//        // 只有在帧末更新 rot_cnt/phase（保证整帧显示使用同一 phase）
//        if (rot_cnt == ROT_PERIOD) begin
//            rot_cnt <= 5'd0;
//            phase <= phase + 3'd1;     // 更新相位
//            phase_reg <= phase + 3'd1; // phase_reg 用于实际颜色选择
//        end else begin
//            rot_cnt <= rot_cnt + 5'd1;
//            // phase_reg 保持不变，等待下一次溢出
//        end
//    end
//end
always@(posedge clk or posedge rst) 
begin if(rst == 1'b1) phase <= 1'b0; 
      else if(rot_cnt == 5'b11111&&h_cnt == H_TOTAL - 1 && v_cnt == V_TOTAL - 1) 
          phase <= phase + 3'd1; 
      else  phase <=phase; 
end 
always@(posedge clk or posedge rst) 
